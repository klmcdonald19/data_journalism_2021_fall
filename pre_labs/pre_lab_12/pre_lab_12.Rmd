...
**Answer**
The data frame is county vaccinations by date. There are 13 columns and 8,530 rows showing vaccination doses, including cumulative doses. 
```{r}
...
**Answer**
Theres a row at the bottom labeled NA with a total of 201. Theres also a lot of repeating numbers for the totals and they're all pretty close. 
...
**Answer**
They basically have all of the same number of records plus or minus a couple. There is also still the NA row that has data.  
...
**Answer**
There is not really a pattern, the dataframe shows that the last 7-10 days are not among the most highly ranked. 
```{r}
...
**Answer**
The chart shows a rapid growth around April and May of people that are getting vaccinated and then a decline in vaccinations in the months following. 
```{r fig.width=9}
ggplot() +
...
**Task** Run the following code and put the value of `sub` in the answer.
**Answer**
The number of people fully vaxxed in PG county on November 14th decreased by -74.25 percent compared to the week before. 
...
**Answer**
The chart is a line chart showing Maryland counties and reported cases. 
```{r}
...
**Task** Run the following code and describe what the resulting chart shows
**Answer**
It is similar to the last chart we produced but much easier to read because of the way the lines are set up. 
```{r}
...
**Answer**
It is a map of Maryland counties and number of cases. It is as of 11/16/21. 
```{r}
ggplot() +
