...
**Answer**
Yes they all have the same column structure because it is the same data. The only difference is what year the data comes from. 
...
**Answer**
There are 195869 rows in maryland_ppp. There are 1057 rows in naics_codes.
```{r}
...
**Answer**
It says there is an error because of incompatible types. 'x$zip' is a character, and 'y$zip' is a double. I think this is because there is a difference in characters between the two of them. You can't join numbers and characters. 
...
**Answer**
No every row does not have demographic data. 
```{r}
maryland_ppp_with_naics_and_demographics <- maryland_ppp_with_naics %>% left_join(maryland_zcta, by=c("zip"="ZCTA5N"))
...
**Answer**
Yes every row now has demographic data.
```{r}
...
**Answer**
The average is 53836.46. The median amount is 20000. 
```{r}
...
**Answer**
The average is 88140.24. The median is 20833. 
```{r}
...
**Answer**
For rural status, the average is 44328.87 and the median is 19621. For urban status, the average is 53868.53 and the mediaan is 20000. 
```{r}
...
**Answer**
For rural status, the average is 73694.14 and the median is 20833. For urban status the average is 90911.14 and the median is 20833.
```{r}
