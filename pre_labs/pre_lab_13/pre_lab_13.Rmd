...
**Answer** There is a table of 5 columns and 51 rows with each row being a state.
```{r}
...
**Answer** There is kind of a pattern. Generally speaking, less Trump voting states got loans at a lower rate.
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** This plot doesn't seem to show as much of a steady pattern than the last one. 
```{r}
...
**Answer** The output shows that the first graph we plotted had a much stronger relationship, while the second one was not as strong. 
```{r}
reveal_data %>%
...
**Answer** The output shows hypotheses, 95 % confidence interval, correlation, and p value. 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** This output has a larger p value than the last one. The r value is also lower which means the correlation is weaker. 
```{r}
...
**Answer** We see a table of two rows, Clinton ans Trump with 3 columns that included mean ppp amount per employee and mean businesses recieving ppp. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** The output is a two sample t test that shows the t value, degrees of freedom, p value, and confidence interval. 
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** In this two sample t test, the t is stronger and p value is lower, which means that there is probably something statistically significant happening. 
```{r}
