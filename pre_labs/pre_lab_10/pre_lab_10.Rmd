...
https://datawrapper.dwcdn.net/SEOSB/1/ 
...
The code is grouping and summarizing to show the county in one column, and the total amount of approved applications in the column next to it. 
```{r}
...
**Answer** Describe what the code below is doing and the output.
The code is basically doing the same thing we just did in the previous code block but it is filtering out the counties that are not a part of Maryland. I  believe it went from 32 to 24. 
```{r}
md_counties <- maryland_ppp %>%
    group_by(project_county_name) %>%
    summarise(count=n()) %>%
...
The column with the fips code was added to the data frame. They all start with 240 except for the last one which is Baltimore city.
```{r}
...
When you join the data you now have the county name and the fip code as well. 
```{r}
...
The map zooms in so you can see the Maryland counties more clearly instead of the entire country. 
...
**Answer** Describe the changes from the previous map
Instead of using the fips code to identify the MD county it says the name of the county when you hover over it. 
...
https://datawrapper.dwcdn.net/lgO48/1/ 