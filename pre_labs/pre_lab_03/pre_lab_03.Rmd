...
**Answer**
It says Warning: One or more passing issues, see 'problems()' for details
```{r}
...
**Answer**
No it showed no parsing errors when it ran. 
```{r}
...
**Answer**
The id field is a number <dbl>. The name field is a character <chr>. The amount field is a number<dbl>. 
```{r}
...
**Answer**:
There are 192959 loans where franchise name is NA. There are 275 loans where the franchise name is Subway. 
```{r}
...
**Answer**:
There are 135073 loans where forgiveness amount is NA. 
```{r}
...
**Answer**:
There are 135073 loans where foregiveness date is NA. It is not the same as the forgiveness account. 
```{r}
...
**Answer**:
There are 135073 loans, which is the same number we found in the previous codeblocks. This means that looking at forgiveness_date and forgiveness_amount together was the same as just running forgiveness_date. They both produced <NA>. 
```{r}
ppp_maryland_loans %>%
...
**Answer**
There were no loans in the data for October 2020. There is a gap in the data. 
```{r}
ppp_maryland_loans %>%
...
**Answer**
The largest loan is for 10 million, which is the maximum number a loan can be. The minimum amount is 6 dollars, which is suspicious because why would a business need to take out a loan of only 6 dollars. 
```{r}
...
**Answer**
There are two loans under 100 dollars. One is for 6 dollars and the other is for 78 dollars. 
```{r}
...
**Answer**
The various fields with amounts do align. There is internal consistency. 
...
**Answer**
There are 6 rows in this data set. 
```{r}
...
**Answer**
The amount column is stored as a character and not a number. 
```{r}
...
**Answer**
One problem is that there is a fully duplicated row (the first and second rows). Another problem is that the street address field is inconsistent. One last problem is that the column headers are inconsistently styled. 
```{r}
arnold_md_loans
```
...
**Answer**
It changed the name to "x1_id" so now it does not start with a number. 
...
**Answer**
The error says "problem with 'summarise()' column. I believe this is because the amount ccolumn is a character and not a number. 
```{r}
# cleaning function
...
**Answer**
There are two duplicates. The business name is Stuart Jones LLC. 
...
There are two mispellings that remain. 
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
...
**Answer**
There is only one mispelling that remains. 
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
...
**Answer**
All of the mispellings are fixed. 
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
