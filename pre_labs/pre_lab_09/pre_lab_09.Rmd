...
**Answer**
It has 195865 rows and 63 columns. 
```{r}
...
**Answer**
We are grouping by project county name, then summarizing by total loans then arranging the top 10 by total loans from high to low. 
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer**
A bar chart is formed with x as the county and the weight on the other axis. 
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
...
**Answer**
It now goes in order of total loans compared to the last codeblock that was not as organized.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
The bar chart is now horizontal. 
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
It changed the background/styling with the minimal theme option. 
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Answer**
The background is now blue. The font is also different. 
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
There is now a title and the font of the chart has changed. 
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
You see a tibble of dates approves and the total loans. 
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
A line chart appears that shows a pattern in the data.
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
We see another line chart but we've used the function scale_x_date() to have one tick mark and label per month.
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
We turned the date label 45 degrees so that it is easy to see and makes the chart more organized.
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
