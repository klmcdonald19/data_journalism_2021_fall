...
**Answer**
This webpage takes you to current national employment statistics.  It talks about how it has switches from NAICS 2012 to NAICS 2017. It also has an overview, history, and conversion methods. 
...
**Answer**
You should see the html code that designed the web page. 
...
**Answer**
The output that appears makes a data option called naics_industry pop up in the environment window. Below the codeblock the output appears as an html document which has a header and body. 
```{r}
# read in the html
...
**Answer**
There are 3 lines underneath naics_industry. It says list of 2. It also has a $ node line, a $ doc line, and a - attr line. 
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
You see a lot when you do this step. You can see the design elements of the page that include things like padding and margins. You also see sepearte elements and styles.
### Task 9: Load image
**Task** Run the following code to display an image showing what you should see on the web page.
```{r, echo=FALSE}
...
**Answer**
The output that appears includes 6 tables. One has conversions, one has sectors, one has divisons, one has SIC, and the last two include positions. 
```{r}
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
It is a list of 6 with tibbles that have a different amount of rows and columns for each line. 
...
**Answer**
You should see 4 rows of conversions from NAICS. This table also includes the reference months implemented and dates released. 
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
You should see a table with two columns showing the sector and description. 
...
**Answer**
Below the codeblock a table appears that shows the section and description. 
```{r}
...
**Answer**
The description section was cleaned. The last row was removed. It is easier to read in this form. 
```{r}
# Read in all html from table, store all tables on page as nested list of dataframes.
naics_industry <- naics_url %>%
  read_html() %>%
...
**Answer** A table is created. In this table we have sector (#) and description (industry).
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
This page shows the utilities sector. Included on this page is also employee statistics. 
...
**Task** Run the following code to read in the html. Briefly describe the output that appears below the codeblock.
**Answer**
Under the codeblock, an html document title shows up. Theres also a <head> and <body>. 
```{r}
...
**Answer**
Below the codeblock is html_node. There is also a class, id, caption, head, body, and foot. 
```{r}
...
**Answer**
A table is produced with 6 rows and 6 columns. Columns include months. 
```{r}
...
**Answer**
Below the codeblock, we now only see employment in June of 2021. 
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
When going to this link you see the Mining, Quarrying, and Oil and Gas Extraction sector. There's tables with trends, prices, injuries, earnings, and other workforce statistics. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
This is the construction sector. Here you see workforce statistic tables that have the same categories that were included in the natural resources sector. 
...
**Answer**
Below the codeblock, all the titles are printed neatly in a column. 
```{r}
...
**Answer**
Nothing appears below the codeblock but the list of sectors shows up in the environment to the right.  
```{r}
...
**Answer**
The sectors are neatly printed in a list of 10. 
```{r}
...
**Answer**
There was an error. The error is object 'sector_list' not found. 
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer**
All the sectors are printed neatly again like they previously were. 
```{r}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer**
There was an error. The error is object 'sector_name' not found. 
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Answer**
The numbers 1-10 appear in a column. They start at 1 and get bigger. 
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
for (number in 1:10) {
  print(number)
...
**Answer**
The result is a table with the sector and description. There are 2 columns and 20 rows. 
```{r}
...
**Answer**
The number 20 appears which tells how many rows are in the previous data frame we created. 
```{r}
nrow(naics_industry)
```
...
**Answer**
This table shows the number of rows indivdually. It starts at 1 and gets greater until it reaches the final 20th row. 
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
...
**Answer**
Below the codeblock is a 1x2 table. The table describes employment in June 2021. 
```{r}
...
**Task** Run the following code to load table from last chapter. Briefly describe the output that appears below the codeblock.
**Answer**
A table of 2 columns and 20 rows is produced. The information inside is the sector and description. 
```{r}
# Define url of page we want to scrape
...
**Answer**
The table is the same as the previous task but there is one difference. This table now has a third column which includes the sector url. 
```{r}
...
**Answer**
This is the same table as the previous task with one added difference. There is now a fourth column called sector_xpath_id. 
```{r}
...
**Answer**
The table is the same as previously but now there are 19 rows since the public administartion sector was removed. 
```{r}
...
**Answer**
There are 19 individual data tables that are produced. Each has their own sector and description. 
```{r}
...
**Answer**
The output we're getting is row numbers q-19. Next to the row numbers includes each url and xpath. 
```{r}
...
**Answer**
19 employment data tables are created with multiple dataframes each. 
```{r}
...
**Answer**
19 employment data tables are created with one dataframe each. 
```{r}
...
**Answer**
19 tables were created again. These differ because they include data seperated by month June-September. 
```{r}
...
**Task** Run the following for loop to clean up tables. Briefly describe the output that appears below the codeblock.
**Answer**
19 tables are created that include sector, description and data from June of 2021. 
```{r}
...
**Answer**
The previous data was combined into a singular data frame. 
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
There is a chart with employment and unemployment data from the months of June- September 2021. 
### Task 37: Load image
**Task** Run the following code to display an image showing what you should see on the web page.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
You should see a table that has unemployment data from June- September 2021. 
### Task 39: Load image
**Task** Run the following code to display an image showing what you should see on the web page.
...
**Answer**
The output that appears under the codeblock removes agriculture from the table. 
```{r}
