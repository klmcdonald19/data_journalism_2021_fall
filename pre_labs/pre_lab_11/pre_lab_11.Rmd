...
**Answer**
A list of documents that include a .shp shapefile as well as a .prj file. 
```{r, echo=FALSE}
...
**Answer**
The columns that exist in this data include things like name, city, address, state, etc. The last column included is called "geometry." 
```{r}
...
**Answer**
It shows we have 72 hospitals. It has 32 fields which includes things like address, city, websites, fips codes, and geometry. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The output shows the kinds of hospitals which include general acute care, psychiatric, military, chronic disease, long term care, rehabilitation, and special hospitals. 
```{r}
md_hospitals %>%
  group_by(TYPE) %>%
  summarise(
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We've narrowed the previous output to only include the 50 general acute care hospitals in Maryland. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We get a series of points, which represent the hospitals, plotted by the latitude and longitude. 
```{r}
md_hospitals %>%
  ggplot() + 
  geom_sf() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We installed the package tigris. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We get 3,233 rows and 18 columns of information. It looks normal but does have the geometry column at the bottom. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We've stored data in md_counties that filtered out only counties in Maryland from the original data. There are 24 rows. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The output is a map of the state of Maryland with the counties. It is plotted by latitude and longitude. 
```{r}
md_counties %>%
  ggplot() + 
  geom_sf() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The output is the same map from before with the dots plotted based on the counties they're in. There are also noticeable clusters of dots. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
There is now different colored dots to show the number of hospital beds. The counties are now white instead of gray. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The output shows that we've filtered out the 50 general acute care hospitals in Maryland. It has read the layer "hospitals" from the data source and theres a bunch of information about name, address, city, state, zip, etc. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The output is that we get the county in Maryland with the total number of hospital beds. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The output shows the MD county with a population estimate. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The output is that we added a geometry category so that we can make a map. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The data didn't join because of capitalization differences between the columns. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We cleaned the data so that now we can try join the two data sets together and there are no problems with capitalization. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We created md_beds_per_100k which is the two data sets joined together, which was made possible by the cleaning we did. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We have now successfully replaced the NAs with values of 0. 
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We did some code to calculate the number of beds per 100000 people in each county (line 414) and then arranged it by highest to lowest. 
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
  mutate(total_beds = replace_na(total_beds,0)) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We have a map of the MD counties. They are colored according to the number of hospital beds per 100000 people. 
```{r}
ggplot() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We've changed the color scale so that it is easier to see differences. 
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
...
**Answer**
We see southern MD has a cluster of the least beds per 100000 people. 
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
