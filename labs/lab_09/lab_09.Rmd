...
axis.text.x = element_text(angle = 45,hjust=1)
 ...
 labs(
    title="Days in April with Most PPP Loans Approved in West Virginia",
    x = "date approved",
    y = "total loans",
    caption = "source: SBA PPP loan database"
 )+
  geom_point(aes(x = date_approved, y = total_loans)) +
   geom_text(aes(x = date_approved, y = total_loans,label=total_loans), nudge_x = -1, nudge_y= 0, check_overlap=T) 
 
```
**A1.** 
April 28th had the most loans with 2,381 loans.
...
```{r}
state_ppp_all <- read.csv("data/state_ppp_all.csv") 

v19 <- load_variables(2019,"acs5", cache = TRUE)

acs_data <- get_acs(geography = "state", variable = "B01001_001")
...
alldata %>%
    ggplot() +
  geom_bar(aes(x=reorder(state, total_ppp_loans), weight=percent), fill= "#2f53da") +
  coord_flip() + 
  theme_minimal()+
 labs(
    title="States With Most PPP Loans per 100,000 People",
    x = "total loans",
    y = "state",
    caption = "source: SBA PPP loan database"
 )
