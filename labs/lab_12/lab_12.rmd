...
**A1.** "The number of children under 12 in Maryland that received their first single dose on November 17 decreased by -65.5 percent compared to the week before and decreased by -49.99 percent compared to yesterday"

```{r}
kids_vaccine_data <- read_csv("https://raw.githubusercontent.com/state-of-maryland/VaccineCSVs/master/MD_COVID19_VaccinationPercentAgeGroupPopulation.csv")
  

kids_vaccine_data <- kids_vaccine_data %>%
  filter(AgeRange == "11 and Under") %>%
  mutate(VACCINATION_DATE = mdy_hms(VACCINATION_DATE)) %>%
arrange(desc(VACCINATION_DATE))

most_recent_day <- kids_vaccine_data %>% slice(1)
day_before <- kids_vaccine_data %>% slice(2)
week_before <- kids_vaccine_data %>% slice(7)
  
 most_recent_day <- most_recent_day %>%
mutate(pct_change_week = (FirstDailyDose - week_before$FirstDailyDose)/week_before$FirstDailyDose*100)

most_recent_day <- most_recent_day %>%
mutate(pct_change_day = (FirstDailyDose -day_before$FirstDailyDose)/day_before$FirstDailyDose*100)
...
**A2.**
All three of the top zip codes are located near Rockville, Maryland, which is near D.C. The top two zipcodes are in the top three most populous zipcodes in MD. Concerning the non-Maryland results, the top three zip codes are all located in D.C., which is not surprising as that is a populous, liberal area which would explain such high vaccination rates.
```{r}
