...
**A1.** The area with the most loans in this industry based on the map is south west, theres a large cluster of orange areas. I notice that the right or east side does not have very many loans in this industry. The county with the most loans is Ritchie. It had 77752742 per 100,000, and the total amount of loans was 7653980. The population is 9844.
```{r}
...
**A2.**
Most of the mine permits, or dots, on this map is in the same places where the highest number of mining loans were on the previous map we just made. This general area of where the dots are clustered are in the same spot I pointed out to be all orange. This must mean there is some correlation between mine permits and loans for the industry. 
```{r}
...
**A3.** As I previously predicted in the last question, most of the dots are in the orange areas that I previously pointed out. The amount of mines are0 in the county that has the most total loan amount per 100k. All the mines are located in the orange area (minus one exception).
```{r}
ggplot() + 
  geom_sf(data=loans_per_100k, aes(fill=amount_per_100k)) + 
  geom_sf(data= expire_dates) +
...
**A4.** Hall Drilling LLC had the biggest chunk. They are an oil drilling company offering construction, trucking, drilling, and product installation. 387 jobs were retained. They might need a loan because no new construction was happening during the pandemic, and a lot of construction projects were halted during this time. 
```{r}
oil_data<-wv_slice_ppp%>%
  filter(str_detect(naics_code, "^21"))%>%
