...
```
**A1.**
https://datawrapper.dwcdn.net/WHdOj/1/ Illinois is the state with the most loans and most undisbursed. 
...
**A2.** https://datawrapper.dwcdn.net/jp1dB/4/ I used a grouped bar chart to show the undisbursed amount compared to the total loans. I think this is a clearer way to show which state has the most undisbursed amounts. The pattern shown is the more undisbursed loans correlate to more total loans. 
```{r}
barber_shops_states <- barber_shops_ppp %>%
group_by(state)%>%
summarise(
...
**A3.**  https://datawrapper.dwcdn.net/Am9wp/1/ They are more common on the East than the West coast. Counties in Florida like Columbia and Polk don't have any data I believe because they are not located on the shoreline and the article we read about said that many fake farms on the NJ shoreline were given Kabbage loans. 
```{r}
...
write_csv(x = alldata3, "alldata3.csv")
```
