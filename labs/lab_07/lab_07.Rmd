...
A1. There are 89 indivdual cases that the U.S. Department of Justice filed against people accused of fraud. 

```{r}
case_fraud_url <- "https://www.justice.gov/criminal-fraud/cares-act-fraud" 
  case_fraud <- case_fraud_url %>%
  read_html() 
 case_fraud
...
A2. There are 28 individual districts. 

```{r}
judicial_district_url<- "https://www.justice.gov/criminal-fraud/cares-act-fraud"
judicial_district <- judicial_district_url %>%
  read_html()
judicial_district
...
A4. He is a rapper who allegedly participated in a scheme to file fraudulent loan applications seeking more than $24 million in forgivable Paycheck Protection Program (PPP) loans guaranteed by the Small Business Administration (SBA) under the Coronavirus Aid, Relief, and Economic Security (CARES) Act. He was charged with another man for wire fraud, bank fraud, and conspiracy to commit wire fraud and bank fraud. Yes I would assign a reporter because of the conspiracy that involved multiple people. I also think that it is more newsworthy because he is a recording artist. 
...
A5. There is 29.78723% of all judicial districts. 
...
A6. One reason could be because the webpage may not be updated. Another reason why they're filed in less than half the country's judicial districts is because the cases may be located only in certain parts of the country.  
...
A7. Wyoming got the most PPP loans per 100,000 population. 
```{r}
loans_per_state <- tibble()

url_52 <- "https://smussenden.github.io/coursefiles/ppp_scraping_example/index.html"
...
loans_per_state <- loans_per_state %>%
