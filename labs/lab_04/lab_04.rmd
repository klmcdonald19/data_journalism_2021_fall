...
The top 5 industries are taxi service, beauty salons, full-service restaurants, offices of real estate agents and brokers, and all other personal services.
...
The top 5 industries are Full-Service Restaurants, Offices of Physicians (except Mental Health Specialists), Plumbing, Heating, and Air-Conditioning Contractors, Offices of Dentists and Commercial and Institutional Building Construction.
...
The zip code withe the largest sum is 20850. It had a total of 136195702.
```{r}
# Write the code to produce your answer here
maryland_ppp_with_naics_and_demographics %>%
  filter(PNHA > 20) %>%
...
20850 is the zip code with the largest sum of loans when the applicant was Asian. The percentage of non-Hispanic Asian population is 23.5 percent. 
```{r}
# Write the code to produce your answer here
maryland_ppp_with_naics_and_demographics %>%
  group_by(title)%>%
...
A lot of loan applicants did not report their race which may explain why there were so little loans applied by Asian applicants. 
...
I think that since applicants explicitly listed themselves as Asian, and the ZCTA data is from a separate data frame, there can be differences. ZCTA might not have gotten exact data from the applicants about their race while the other data specifically asked applicants. 
```{r}
# Write the code to produce your answer here
maryland_ppp_with_naics_and_demographics %>%
  filter(race== "Asian") %>%
...
44 industries have exactly 1 loan in the data. I think that the most newsworthy industry to look into is Casinos. I think that Casinos are such a huge industry and were hit hard during the pandemic due to closures. This makes me wonder why only 1 loan to a casino occurred in the data since it is such a big industry. 
```{r}
# Write the code to produce your answer here
maryland_ppp_with_naics_and_demographics %>%
  group_by(title)%>%
