...
The largest percentage of international migration is the Suburban Washington Region. 
```{r}
...
I mutated the data to find the percent of the total, which is the total divided by 198996. Then we had to arrange by the highest percentage to the lowest percentage. It is the Suburban Washington Region.  
...
The largest percentage change was by Cecil County. It had a percentage change of -1.279. 
```{r}
# Write the code to produce your answer here
international_migration %>%
  mutate(
    percent_change= (July_2018 - July_2017) / July_2017
) %>%
...
There could have been lower levels of immigration due to the fact that Trump was in office. He created laws that made it harder for immigration into the country while he was president, so this could be a reason there were a lot of changes. 
...
Baltimore City has the largest net decrease.
```{r}
# Write the code to produce your answer here
domestic_migration <- domestic_migration %>% 
arrange(total)
...
18 regions had a net positive migration during this time. 
...
This data is pretty split in terms of positive and negative migration. Frederick County has the highest positive migration. Something that stood out to me is the total for Maryland was negative. Most of the positive migrations are on the Eastern Shore which have relatively low populations. 
